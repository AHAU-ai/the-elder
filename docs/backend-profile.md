...
- **LLM:** Anthropic SDK direct (`@anthropic-ai/sdk`), no LangChain/framework layer. `lib/model.config.ts` carries a `LLM_PROVIDER`/`resolveModel()` multi-provider abstraction (Gemini/OpenAI fallback constants) that is deliberate, allowlisted scaffolding for future multi-provider support — `scripts/check-unwired-exports.mjs` flags it as unwired and its allowlist entry says so explicitly (`"multi-provider scaffolding; app currently uses PRIMARY_MODEL directly, not yet adopted"`). `divine/route.ts`'s actual generation and welfare-judge calls use `PRIMARY_MODEL`/`WELFARE_MODEL` directly and always have; no Gemini/OpenAI SDK is installed yet, so adopting it is real, separate work, not a config flip.
...
- **`lib/tierLedger.ts`'s `deriveEffectiveTier()` is now the single source of truth** for tier entitlement math, called once by `getEffectiveTier()` and once directly by `divine/route.ts` from a pre-fetched `TierRecord` — removed a duplicate `elder_user` row fetch that used to happen twice per request (2026-09-23).
