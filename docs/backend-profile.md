...
- **LLM:** Anthropic SDK direct (`@anthropic-ai/sdk`), no LangChain/framework layer, no other provider. (A `LLM_PROVIDER`/multi-provider abstraction previously lived in `lib/model.config.ts` but had zero callers and no Gemini/OpenAI SDK installed to back it — removed 2026-09-23, see that file's own comment.)
